    ...
    public partial class frmMainMenu : Form
    {
        ...
        public void GetData()
        {
            try
            {
				...
				String sql = "SELECT Product.ProductID,ProductName,Features,Price,sum(temp_stock.Quantity),sum(Price * temp_stock.Quantity),SupplierName from Temp_Stock,Product,Supplier where Temp_Stock.ProductID = Product.ProductID group by Product.productID,productname,Price,Features,temp_stock.Quantity,Supplier.SupplierName having(temp_stock.Quantity > 0) order by ProductName";
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            con = new SqlConnection(cs.DBConn);
                con.Open();
				//String sql = "SELECT Product.ProductID,ProductName,Features,Price,sum(temp_stock.Quantity),sum(Price*temp_stock.Quantity),SupplierName from Temp_Stock,Product,Supplier where Temp_Stock.ProductID=Product.ProductID and ProductName like '" + txtProductName.Text + "%' group by product.ProductID,productname,Price,Features,Quantity having(quantity>0) order by ProductName";
				String sql = "SELECT Product.ProductID,ProductName,Features,Price,sum(temp_stock.Quantity),sum(Price * temp_stock.Quantity),SupplierName from Temp_Stock,Product,Supplier where Temp_Stock.ProductID=Product.ProductID and ProductName like '" + txtProductName.Text + "%' group by product.ProductID,productname,Price,Features,Quantity,Supplier.SupplierName having(quantity>0) order by ProductName";
